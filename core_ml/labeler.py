...
class JCORESMiner:
    ...
    # CSF-A depth: JCORES prints depths as e.g. '207.45 m CSF-A'
    ...
    def extract(self, backbone):
        ...
        for i in range(len(doc)):
            ...
            # 1. Extract core ID — JCORES pattern
            core_match = self.CORE_PATTERN.search(text)
            if core_match:
                last_core = core_match.group(1).upper()
            core_id = last_core   # carry forward if not on this page
            ...
            if core_id and section_id:
                ...
                if depth is None and section_id != 'CC':
                    # Try without the core type suffix (e.g., '1W' -> '1')
